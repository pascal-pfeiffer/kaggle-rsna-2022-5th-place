...
class Net(nn.Module):
    ...
    def forward_backbone(self, x, ids):
        ...
        xx = torch.zeros(
            (len(ids), xxx.size(1), xxx.size(2), xxx.size(3)),
            device=xxx.device,
            dtype=xxx.dtype,
        )
        xx[idx] = xxx
    ...
    def forward(
        ...
        if np.random.random() <= self.cfg.mixup_probability and self.training:
            x, targets, vert_targets = self.mixup(x, targets, vert_targets)
        ...
        if calculate_loss:
            ...
            if self.training and self.cfg.sample_weights:
                ...
                if self.cfg.sample_weights_global:
                    loss = loss.mean()
                else:
                    loss = loss.sum()
                    loss = loss / batch["weight"].sum()
            else:
                loss = loss.mean()
